...
if [[ -z $1 || -z $2 ]]; then
  ##### DISCOVERY #####
  PROCESSORS=`sensors 2>&1 | awk '{if (tolower($1)~"physical") {counter +=1;}} END {for (i=1; i<=counter; i+=1) printf("CPU%1d\n", i-1); }'`
  ...
  HOST="$1"
  CPUNAME="$2"
  TABLE=`sensors 2>&1 | awk '{if (tolower($1)~"physical") { counter +=1; } if (tolower($1)=="core")
