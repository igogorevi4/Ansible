...
NOWTIME=$(date +%s)
if [ ! -f $FILESTATUS ]
then wget http://127.0.0.1/php-status -O $FILESTATUS -o /dev/null
fi
FILETIME=$(stat -c %Y $FILESTATUS)
let "TIME = $NOWTIME - $FILETIME"
if [ "$TIME" -ge "20" ]
then
wget http://127.0.0.1/php-status -O $FILESTATUS -o /dev/null
fi
...
;;
ping)
wget http://127.0.0.1/ping -O $FILEPING -o /dev/null
grep "^1" $FILEPING
;;
